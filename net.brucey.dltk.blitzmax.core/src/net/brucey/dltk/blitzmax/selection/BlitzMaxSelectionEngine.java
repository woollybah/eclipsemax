...
public class BlitzMaxSelectionEngine extends ScriptSelectionEngine {
	...
	private void selectionOnTypeDeclaration(ModuleDeclaration parsedUnit,
		...
		if (start < typeDeclaration.getNameStart()
		    || start > typeDeclaration.getNameEnd()) {
			return;
		}
	...
	private void selectionOnMethodDeclaration(ModuleDeclaration parsedUnit,
		...
		if (methodDeclaration instanceof BlitzMaxMethodDeclaration) {
			if (start < ((BlitzMaxMethodDeclaration) methodDeclaration).getFunc()
			    .sourceStart()
			    || start > ((BlitzMaxMethodDeclaration) methodDeclaration).getFunc()
			        .sourceEnd()) {
				return;
			}
		}
