...
public class ExtendedVariableReference extends Reference {
	...
	/**
	 * Adding expression to extended variable. Use only in parsers then construct.
	 * 
	 * @param expr
	 */
	...
	public boolean isCall(int index) {
		...
		if (index < expressions.size() - 1) {
			Expression afterIndexExpression = (Expression) expressions.get(index + 1);
		...
		if (expressions != null) {
			...
			while (i.hasNext()) {
				...
				if (bFirst) {
					...
					if (!(expr instanceof CallHolder) && !(expr instanceof IndexHolder)) {
